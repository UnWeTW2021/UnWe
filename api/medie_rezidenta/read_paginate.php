...
$stmt = $MedieRezidenta->readOrderJudet();
$num = $stmt->rowCount();
...
if($num > 0) {
    ...
    if (isset($_GET['page']) && isset($_GET['number'])) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            array_push($data_arrALL["medie_rezidenta"], $data_item);
        }

        for ( $index ; $index < $number*$page+$number ; $index++)
        {
            if($index<count($data_arrALL['medie_rezidenta']) && $data_arrALL['medie_rezidenta'][$index]['JUDET']!='TOTAL') {

                array_push($data_arr['medie_rezidenta'], $data_arrALL['medie_rezidenta'][$index-$number]);
            }
        }
